...
import { AsyncStorage, Dimensions, Image, Alert, ImageBackground, SafeAreaView, ScrollView, StyleSheet, TouchableOpacity } from 'react-native';
...
const BuyOrdersScreen = ({ navigation, route }) => {
    ...
    useEffect(() => {
      AsyncStorage.getItem("coin").then((coin) => {
        fetch(`http://146.190.27.11/User/PeerTxn/getUsers.php?coin=${coin}`, {
          method: "GET"
        })
        .then(response => response.json())
        .then((data) => {
          setP2pUsers(data.traders);
        })
      })
    }, [])
    return (
            ...
            <View style={{ marginTop: 40 }}/>
                ...
                <ScrollView>
                  ...
                  <View style={styles.cardsRow}>
                    {
                      p2pUsers.map((obj, index) => {
                        return(
                          <View key={index} style={[styles.tradeCard]}>
                            <TouchableOpacity onPress={ () => {
                              AsyncStorage.getItem(obj.wallet.address).then((min) => {
                                if(parseInt(min) < 0){
                                  Alert.alert("Ouch", "This user cannot sell crypto presently.")
                                }else if(parseInt(min) == 0){
                                  AsyncStorage.setItem("trader", JSON.stringify(obj)).then(() => {
                              ...
                              <OrderCard navigation={navigation} data={obj} status={ params.status }></OrderCard>
                            </TouchableOpacity>
                          </View>
                        )
                      })
